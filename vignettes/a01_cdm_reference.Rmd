...
Each one of these tables is associated with a set of fields. For example, for the `person` table has includes:
...
tables <- standardTables(version = version)
...
All of these tables may contain additional fields.

### 2) Cohort tables

Studies using the OMOP CDM often create study-specific cohort tables. We also consider these as part of the cdm reference once created. Each cohort table is associated with a specific class of its own, a `generatedCohortSet`, which is described more in a subsequent vignette.

As with the standard OMOP CDM tables, cohort tables are expected to contain a specific set of fields (with no restriction placed on whether they include additional fields or not).
...
The [Achilles R](https://ohdsi.github.io/Achilles) package provides descriptive statistics on an OMOP CDM database. The results from Achilles are stored in tables in the database. The following tables are created with the given columns.
